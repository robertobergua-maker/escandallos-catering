...
def _leer_cookie_refresh_token():
    ...
    # 1) Streamlit expone las cookies de la petición actual de forma síncrona.
    try:
    ...
    # 2) Fallback: CookieManager puede tardar una ejecución en hidratarse,
    # pero cuando está disponible permite leer cookies del navegador.
    try:
        gestor = obtener_cookie_manager()
        if gestor is not None:
            try:
                cookies = gestor.get_all()
            except Exception:
                cookies = None
            if isinstance(cookies, dict):
                valor = cookies.get(AUTH_REFRESH_COOKIE)
                if valor:
                    return str(valor)
...
def _inyectar_cookie_refresh_token(refresh_token):
    """Escribe la cookie desde el navegador como respaldo del CookieManager."""
    token = str(refresh_token or "").strip()
    if not token:
        return
    try:
        nombre_js = json.dumps(AUTH_REFRESH_COOKIE)
        token_js = json.dumps(token)
        components.html(
            f"""
            <script>
            (function() {{
              const name = {nombre_js};
              const value = encodeURIComponent({token_js});
              const secure = window.location.protocol === "https:" ? "; Secure" : "";
              document.cookie = name + "=" + value + "; Max-Age=2592000; Path=/; SameSite=Lax" + secure;
            }})();
            </script>
            """,
            height=0,
...
def _guardar_cookie_refresh_token(refresh_token):
    ...
    # Respaldo: escribe la cookie mediante JS. En el siguiente F5,
    # st.context.cookies debería recibirla de forma síncrona.
    _inyectar_cookie_refresh_token(token)
    return True or guardado
...
def restaurar_sesion_supabase():
    ...
    # En algunos navegadores el componente de cookies necesita una primera
    # ejecución para hidratarse. Si no vemos cookie en la petición actual,
    # montamos el gestor y hacemos un único rerun controlado antes de mostrar login.
    if not refresh_token and stx is not None and not st.session_state.get("_auth_cookie_retry_done"):
        st.session_state["_auth_cookie_retry_done"] = True
        try:
            obtener_cookie_manager()
        except Exception:
            pass
        try:
            st.rerun()
    ...
    try:
        ...
        _guardar_sesion_supabase(usuario, sesion)
        st.session_state["_auth_cookie_retry_done"] = False
        asegurar_usuario_app_supabase()
        return True
    except Exception:
        return False
...
if 'ingrediente_ficha_modo' not in st.session_state:
    st.session_state['ingrediente_ficha_modo'] = "sin_seleccion"
if 'ingrediente_ficha_indice' not in st.session_state:
    st.session_state['ingrediente_ficha_indice'] = None
if 'ingrediente_ficha_datos' not in st.session_state:
    st.session_state['ingrediente_ficha_datos'] = {}
if 'ingrediente_ficha_revision' not in st.session_state:
...
@st.cache_data(ttl=600)
def cargar_alergenos_supabase():
    """Carga el catálogo normalizado de alérgenos activos."""
    columnas = ALERGENOS_COLUMNAS
    if not supabase_disponible or supabase is None:
...
def cargar_ingrediente_alergenos_supabase(trigger=0):
    ...
    try:
        respuesta = (
            supabase
            .table("ingrediente_alergenos")
            .select("codigo_ingrediente,alergeno_id")
            .execute()
        )
        relaciones = pd.DataFrame(respuesta.data or [])
        if relaciones.empty:
            return pd.DataFrame(columns=columnas)
        for col in ["codigo_ingrediente", "alergeno_id"]:
            if col not in relaciones.columns:
                relaciones[col] = ""
        relaciones["codigo_ingrediente"] = relaciones["codigo_ingrediente"].fillna("").astype(str).str.strip().str.upper()
        relaciones["alergeno_id"] = relaciones["alergeno_id"].fillna("").astype(str)

        catalogo = cargar_alergenos_supabase()
        if catalogo.empty:
            relaciones["codigo"] = ""
            relaciones["nombre"] = ""
            return relaciones[columnas].copy()
        catalogo_min = catalogo[["id", "codigo", "nombre"]].rename(columns={"id": "alergeno_id"})
        df = relaciones.merge(catalogo_min, on="alergeno_id", how="left")
        for col in columnas:
            if col not in df.columns:
                df[col] = ""
        return df[columnas].copy()
...
def limpiar_cache_alergenos():
    for funcion in (cargar_alergenos_supabase, cargar_ingrediente_alergenos_supabase):
        try:
            funcion.clear()
        except Exception:
            pass
...
def _alergeno_item_desde_fila(fila):
    return {
        "id": str(fila.get("alergeno_id", fila.get("id", "")) or ""),
        "codigo": str(fila.get("codigo", "") or "").strip().upper(),
        "nombre": str(fila.get("nombre", "") or "").strip(),
    }


def obtener_mapa_alergenos_ingredientes(codigos_ingrediente=None):
    """Devuelve {CODIGO_INGREDIENTE: [alérgenos...]} para los códigos pedidos."""
    df = cargar_ingrediente_alergenos_supabase(st.session_state.get("db_trigger", 0))
    mapa = {}
    if df.empty:
        return mapa
    codigos_filtrar = None
    if codigos_ingrediente is not None:
        codigos_filtrar = {str(c or "").strip().upper() for c in codigos_ingrediente if str(c or "").strip()}
    for _, fila in df.iterrows():
        codigo_ing = str(fila.get("codigo_ingrediente", "") or "").strip().upper()
        if not codigo_ing or (codigos_filtrar is not None and codigo_ing not in codigos_filtrar):
            continue
        item = _alergeno_item_desde_fila(fila)
        if not item["id"] and not item["codigo"] and not item["nombre"]:
            continue
        mapa.setdefault(codigo_ing, [])
        if item not in mapa[codigo_ing]:
            mapa[codigo_ing].append(item)
    return mapa


def cargar_alergenos_ingrediente(codigo_ingrediente):
    codigo = str(codigo_ingrediente or "").strip().upper()
    if not codigo:
        return []
    return obtener_mapa_alergenos_ingredientes([codigo]).get(codigo, [])


def formatear_alergenos(alergenos):
    if alergenos is None:
        return "Sin alérgenos declarados"
    nombres = []
    for item in alergenos:
        if isinstance(item, dict):
            nombre = str(item.get("nombre") or item.get("codigo") or "").strip()
        else:
            nombre = str(item or "").strip()
        if nombre and nombre not in nombres:
            nombres.append(nombre)
    return ", ".join(nombres) if nombres else "Sin alérgenos declarados"


def texto_alergenos_ingrediente(ingrediente):
    return formatear_alergenos((ingrediente or {}).get("alergenos", []))


def alergeno_ids_desde_nombres(nombres):
    nombres_set = {str(n or "").strip() for n in nombres or [] if str(n or "").strip()}
    if not nombres_set:
        return []
    catalogo = cargar_alergenos_supabase()
    if catalogo.empty:
    ...
    for _, fila in catalogo.iterrows():
        if str(fila.get("nombre", "") or "").strip() in nombres_set:
            ids.append(str(fila.get("id", "") or ""))
    return [item for item in ids if item]


def enriquecer_ingrediente_con_alergenos(ingrediente):
    datos = dict(ingrediente or {})
    codigo = str(datos.get("codigo", datos.get("codigo_ingrediente", "")) or "").strip().upper()
    alergenos = datos.get("alergenos")
    if not isinstance(alergenos, list):
        alergenos = []
    if not alergenos and codigo_ingrediente_valido(codigo):
        alergenos = cargar_alergenos_ingrediente(codigo)
    datos["alergenos"] = alergenos
    datos["alergeno_ids"] = [str(item.get("id")) for item in alergenos if isinstance(item, dict) and item.get("id")]
    datos["alergenos_texto"] = formatear_alergenos(alergenos)
    return datos


def enriquecer_inventario_dict_con_alergenos():
    if not inventario_dict:
        return
    mapa = obtener_mapa_alergenos_ingredientes(inventario_dict.keys())
    for codigo, item in inventario_dict.items():
        alergenos = mapa.get(str(codigo).strip().upper(), [])
        item["alergenos"] = alergenos
        item["alergeno_ids"] = [a.get("id") for a in alergenos if a.get("id")]
        item["alergenos_texto"] = formatear_alergenos(alergenos)


def guardar_alergenos_ingrediente(codigo_ingrediente, alergeno_ids):
    """Reemplaza las relaciones de alérgenos de un ingrediente."""
    codigo = str(codigo_ingrediente or "").strip().upper()
    ...
    if not supabase_disponible or supabase is None:
        return False, "El inventario no está conectado correctamente."
    ids_limpios = []
    for alergeno_id in alergeno_ids or []:
        alergeno_id = str(alergeno_id or "").strip()
        if alergeno_id and alergeno_id not in ids_limpios:
            ids_limpios.append(alergeno_id)
    try:
        (
            supabase
            .table("ingrediente_alergenos")
            .delete()
            .eq("codigo_ingrediente", codigo)
            .execute()
        )
        if ids_limpios:
            filas = [
                {"codigo_ingrediente": codigo, "alergeno_id": alergeno_id}
                for alergeno_id in ids_limpios
            ]
            supabase.table("ingrediente_alergenos").insert(filas).execute()
        limpiar_cache_alergenos()
        st.session_state["db_trigger"] += 1
        return True, ""
    except Exception as exc:
        return False, f"No se pudieron guardar los alérgenos del ingrediente: {exc}"


def calcular_alergenos_desde_ingredientes(ingredientes):
    resultado = []
    vistos = set()
    codigos_sin_alergenos = []
    for ingrediente in ingredientes or []:
        datos = dict(ingrediente or {})
        alergenos = datos.get("alergenos") if isinstance(datos.get("alergenos"), list) else []
        codigo = str(datos.get("codigo", "") or "").strip().upper()
        if not alergenos and codigo_ingrediente_valido(codigo):
            codigos_sin_alergenos.append(codigo)
        for alergeno in alergenos:
            item = alergeno if isinstance(alergeno, dict) else {"id": "", "codigo": "", "nombre": str(alergeno)}
            clave = str(item.get("id") or item.get("codigo") or item.get("nombre") or "").strip().upper()
    ...
    if codigos_sin_alergenos:
        mapa = obtener_mapa_alergenos_ingredientes(codigos_sin_alergenos)
        for codigo in codigos_sin_alergenos:
            for item in mapa.get(codigo, []):
                clave = str(item.get("id") or item.get("codigo") or item.get("nombre") or "").strip().upper()
                if clave and clave not in vistos:
                    vistos.add(clave)
                    resultado.append(item)
    return sorted(resultado, key=lambda item: str(item.get("nombre", "")).lower())


# Enriquecer inventario en memoria con los alérgenos normalizados disponibles.
enriquecer_inventario_dict_con_alergenos()
...
def datos_ficha_ingrediente(ingrediente=None):
    ingrediente = enriquecer_ingrediente_con_alergenos(dict(ingrediente or {}))
    ...
    return {
        ...
        "familia": str(ingrediente.get("familia", "SIN CLASIFICAR") or "SIN CLASIFICAR").strip(),
        "unidad_medida": unidad,
        "merma": numero_seguro(ingrediente.get("merma", 0.0), 0.0),
        ...
        "alergenos": ingrediente.get("alergenos", []),
        "alergeno_ids": ingrediente.get("alergeno_ids", []),
        "alergenos_texto": ingrediente.get("alergenos_texto", "Sin alérgenos declarados"),
    }


def activar_ficha_ingrediente(modo, ingrediente=None, indice=None):
    st.session_state["ingrediente_ficha_modo"] = modo
    st.session_state["ingrediente_ficha_indice"] = indice
    st.session_state["ingrediente_ficha_datos"] = datos_ficha_ingrediente(ingrediente)
    st.session_state["ingrediente_fila_seleccionada"] = indice if modo == "fila_receta" else None
...
def validar_datos_ficha_ingrediente(datos, requiere_cantidad=False):
    ...
    if numero_seguro(datos.get("precio_unidad", 0.0), 0.0) < 0:
        return False, "El precio no puede ser negativo."
    return True, ""


def guardar_ficha_en_inventario(datos):
    if not supabase_disponible or supabase is None:
        return False, "El inventario no está conectado.", None

    codigo = str(datos.get("codigo", "") or "").strip().upper()
    if not codigo_ingrediente_valido(codigo):
        codigo = generar_codigo_ingrediente_nuevo(
            datos.get("descripcion", ""),
            set(inventario_dict.keys()),
        )
    ...
    try:
        ...
        ok_alergenos, mensaje_alergenos = guardar_alergenos_ingrediente(
            codigo,
            datos.get("alergeno_ids", []),
        )
        if not ok_alergenos:
            return False, mensaje_alergenos, None
        st.session_state["db_trigger"] += 1
...
def anadir_ficha_a_receta(datos):
    ingrediente = datos_ficha_ingrediente(datos)
    ...
    if codigo_ingrediente_valido(codigo):
        for idx, existente in enumerate(st.session_state["ingredientes"]):
            if str(existente.get("codigo", "")).strip().upper() == codigo:
                cantidad_anterior = numero_seguro(existente.get("cantidad_bruta", 0.0), 0.0)
                ingrediente["cantidad_bruta"] += cantidad_anterior
                st.session_state["ingredientes"][idx] = ingrediente
...
def actualizar_fila_receta_desde_ficha(datos):
    idx = st.session_state.get("ingrediente_ficha_indice")
    if idx is None or not (0 <= int(idx) < len(st.session_state["ingredientes"])):
        return False, "La fila seleccionada ya no existe."
    ingrediente_actualizado = datos_ficha_ingrediente(datos)
    codigo_actualizado = ingrediente_actualizado["codigo"]
    if codigo_ingrediente_valido(codigo_actualizado):
        for otro_idx, ingrediente in enumerate(st.session_state["ingredientes"]):
            if otro_idx == int(idx):
                continue
            if str(ingrediente.get("codigo", "")).strip().upper() == codigo_actualizado:
                return False, "Ese código ya está en otra fila de la receta. Edita la fila existente."
    st.session_state["ingredientes"][int(idx)] = ingrediente_actualizado
    activar_ficha_ingrediente("fila_receta", st.session_state["ingredientes"][int(idx)], int(idx))
    marcar_receta_modificada_manualmente()
    return True, "Fila de receta actualizada."
...
ALERGENOS_COLUMNAS = ["id", "codigo", "nombre", "descripcion", "activo", "created_at"]
INGREDIENTE_ALERGENOS_COLUMNAS = ["id", "codigo_ingrediente", "alergeno_id", "created_at"]
...
def calcular_ingredientes_ajustados_para_menu(ingredientes, raciones_menu, raciones_base):
    """Ajusta cantidades de una receta para las raciones asignadas en el menú."""
    raciones_menu = numero_seguro(raciones_menu, 0.0)
    raciones_base = numero_seguro(raciones_base, 1.0)
    if raciones_base <= 0:
        raciones_base = 1.0
    factor = raciones_menu / raciones_base
    filas = []
    for ingrediente in ingredientes or []:
        ing = enriquecer_ingrediente_con_alergenos(ingrediente)
        cantidad_base = numero_seguro(ing.get("cantidad_bruta", 0.0), 0.0)
        precio_unidad = numero_seguro(ing.get("precio_unidad", 0.0), 0.0)
        cantidad_ajustada = cantidad_base * factor
        filas.append({
            "codigo": str(ing.get("codigo", "") or ""),
            "ingrediente": str(ing.get("descripcion", "") or ""),
            "cantidad_base": cantidad_base,
            "cantidad_ajustada": cantidad_ajustada,
            "unidad": str(ing.get("unidad_medida", "") or ""),
            "merma_pct": numero_seguro(ing.get("merma", 0.0), 0.0),
            "precio_unidad": precio_unidad,
            "coste_ajustado": cantidad_ajustada * precio_unidad,
            "alergenos": texto_alergenos_ingrediente(ing),
        })
    return filas


def calcular_alergenos_ficha_receta(ingredientes):
    return calcular_alergenos_desde_ingredientes(ingredientes)


def calcular_alergenos_menu(fichas):
    vistos = set()
    resultado = []
    for ficha in fichas or []:
        for item in ficha.get("alergenos", []):
            clave = str(item.get("id") or item.get("codigo") or item.get("nombre") or "").strip().upper()
            if clave and clave not in vistos:
                vistos.add(clave)
                resultado.append(item)
...
def construir_ficha_tecnica_receta_menu(receta, ingredientes, raciones_menu):
    receta = dict(receta or {})
    ingredientes_lista = list(ingredientes or [])
    raciones_base = numero_seguro(receta.get("raciones_base", 1.0), 1.0)
    ingredientes_ajustados = calcular_ingredientes_ajustados_para_menu(
        ingredientes_lista,
        raciones_menu,
        raciones_base,
    )
    alergenos = calcular_alergenos_ficha_receta(ingredientes_lista)
    return {
        "receta_id": receta.get("id"),
        "codigo_receta": receta.get("codigo_receta", ""),
        "nombre_receta": receta.get("nombre", "Receta sin nombre"),
        "raciones_menu": numero_seguro(raciones_menu, 0.0),
        "raciones_base": raciones_base,
        "ingredientes": ingredientes_ajustados,
        "elaboracion": str(receta.get("elaboracion", "") or "").strip(),
        "observaciones": str(receta.get("observaciones", "") or "").strip(),
        "alergenos": alergenos,
        "alergenos_texto": formatear_alergenos(alergenos),
    }


def cargar_fichas_tecnicas_menu_desde_lineas(lineas_menu):
    fichas = []
    for linea in normalizar_lineas_menu(lineas_menu or []):
        receta_id = str(linea.get("receta_id", "") or "").strip()
        if not receta_id:
            continue
        cabecera, ingredientes_df = cargar_receta_detalle_supabase(receta_id)
        if not cabecera:
            continue
        ingredientes = preparar_ingredientes_receta_para_sesion(ingredientes_df)
        raciones_menu = numero_seguro(
            linea.get("raciones_receta_en_menu", linea.get("raciones", 0.0)),
            0.0,
        )
        fichas.append(construir_ficha_tecnica_receta_menu(cabecera, ingredientes, raciones_menu))
    return fichas


def cargar_fichas_tecnicas_menu(menu_id):
    cabecera, lineas_df = cargar_menu_detalle_supabase(menu_id)
    if lineas_df is None or lineas_df.empty:
        return cabecera, []
    lineas = normalizar_lineas_menu(lineas_df.to_dict(orient="records"))
...
def preparar_ingredientes_receta_para_sesion(ingredientes_df):
    ...
    for _, fila in ingredientes_df.iterrows():
        ...
        ingredientes.append(enriquecer_ingrediente_con_alergenos({
...
with st.sidebar:
    ...
    st.markdown("#### 🧾 Ficha de ingrediente")
    modo_ficha = st.session_state.get("ingrediente_ficha_modo", "sin_seleccion")
    etiquetas_modo = {
        "sin_seleccion": "Sin selección",
        "fila_receta": "Editando fila de receta",
        "inventario": "Ingrediente de inventario",
    ...
    if modo_ficha == "sin_seleccion":
        ...
        with st.form(f"form_ficha_ingrediente_{revision_ficha}"):
            ...
            catalogo_alergenos_ficha = cargar_alergenos_supabase()
            opciones_alergenos_ficha = catalogo_alergenos_ficha["nombre"].dropna().astype(str).tolist() if not catalogo_alergenos_ficha.empty else []
            nombres_alergenos_actuales = [
                str(item.get("nombre", "") or "").strip()
                for item in ficha.get("alergenos", [])
                if isinstance(item, dict) and str(item.get("nombre", "") or "").strip()
            ]
            nombres_alergenos_actuales = [
                nombre for nombre in nombres_alergenos_actuales
                if nombre in opciones_alergenos_ficha
            ]
            nombres_alergenos_ficha = st.multiselect(
                "Alérgenos",
                options=opciones_alergenos_ficha,
                default=nombres_alergenos_actuales,
                help="Catálogo normalizado desde Supabase. Se guarda en ingrediente_alergenos.",
            )
            alergeno_ids_ficha = alergeno_ids_desde_nombres(nombres_alergenos_ficha)
            ...
            datos_editados_ficha = {
                ...
                "alergeno_ids": alergeno_ids_ficha,
                "alergenos": [
                    {"id": alergeno_id, "codigo": "", "nombre": nombre}
                    for alergeno_id, nombre in zip(alergeno_ids_ficha, nombres_alergenos_ficha)
                ],
                "alergenos_texto": formatear_alergenos(nombres_alergenos_ficha),
            }

            accion_col1, accion_col2 = st.columns(2)
            with accion_col1:
                anadir_receta_ficha = st.form_submit_button(
                    "Añadir a receta",
                    use_container_width=True,
            ...
            eliminar_fila_ficha = st.form_submit_button(
                "Eliminar de esta receta",
                use_container_width=True,
                disabled=modo_ficha != "fila_receta",
            )

            if anadir_receta_ficha:
                valido, mensaje = validar_datos_ficha_ingrediente(datos_editados_ficha, requiere_cantidad=True)
                if not valido:
                    st.error(mensaje)
                else:
                ...
                valido, mensaje = validar_datos_ficha_ingrediente(datos_editados_ficha)
                if not valido:
                    st.error(mensaje)
                else:
                    ok, mensaje = actualizar_fila_receta_desde_ficha(datos_editados_ficha)
                    if ok:
            ...
            elif eliminar_fila_ficha:
                idx_eliminar = st.session_state.get("ingrediente_ficha_indice")
                if idx_eliminar is None or not (0 <= int(idx_eliminar) < len(st.session_state["ingredientes"])):
                    st.error("La fila seleccionada ya no existe.")
                else:
                    st.session_state["ingredientes"].pop(int(idx_eliminar))
                    limpiar_ficha_ingrediente()
                    marcar_receta_modificada_manualmente()
                    st.session_state["aviso_ingrediente"] = (
                        "Ingrediente eliminado solo de esta receta. El inventario no se ha modificado."
                    )
                    st.rerun()
...
ADMIN_TABLAS_BBDD = {
    "alergenos": {
        ...
        "pk": "id",
        "order": "nombre",
        "columns": ALERGENOS_COLUMNAS,
        "generated_pk": True,
    },
    "ingrediente_alergenos": {
        "label": "Alérgenos por ingrediente",
        "pk": "id",
        "order": "codigo_ingrediente",
        "columns": INGREDIENTE_ALERGENOS_COLUMNAS,
        "generated_pk": True,
    },
    "inventario": {
        "label": "Inventario",
...
with main_tab_recetas:
    ...
    for idx, ing in enumerate(ingredientes_activos):
        datos_filas.append({
            ...
            "alergenos_texto": texto_alergenos_ingrediente(enriquecer_ingrediente_con_alergenos(ing)),
        })
    ...
    datos_filas.append({
        ...
        "alergenos_texto": texto_alergenos_ingrediente(borrador_alta),
    })
    ...
    df_display["alergenos_texto"] = df_display["alergenos_texto"].fillna("Sin alérgenos declarados").astype(str)
    df_display["coste_total"] = (df_display["cantidad_bruta"] * df_display["precio_unidad"]).astype(float)

    df_display = df_display[
        ["seleccionar", "descripcion", "cantidad_bruta_txt", "unidad_medida", "merma_txt", "precio_unidad_txt", "coste_total", "alergenos_texto", "codigo"]
    ]
    st.caption(
    ...
    tabla_ingredientes = st.data_editor(
        ...
        column_config={
            ...
            "alergenos_texto": st.column_config.TextColumn("Alérgenos", width="medium"),
            "codigo": st.column_config.TextColumn("Código"),
        },
        disabled=["coste_total", "alergenos_texto"],
    ...
    for idx in range(len(ingredientes_activos)):
        ...
        if codigo_ingrediente_valido(codigo_editado) and codigo_editado in inventario_dict:
            original = enriquecer_ingrediente_con_alergenos({
                **inventario_dict.get(codigo_editado, {}),
                **original,
            })
    ...
    if alergenos_receta_actual:
        ...
        st.info("Sin alérgenos declarados")
...
with main_tab_menus:
    ...
    st.markdown("#### Fichas técnicas del menú")
    if lineas_menu_actual:
        ...
        st.download_button(
            ...
            data=generar_excel_fichas_tecnicas_menu(nombre_menu_excel, fichas_menu_actual),
            file_name=f"fichas_tecnicas_{nombre_menu_excel.replace(' ', '_')}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True,
        )
        for ficha in fichas_menu_actual:
            titulo_ficha = (
                f"Ficha técnica · {ficha.get('nombre_receta', 'Receta sin nombre')} · "
                f"{ficha.get('raciones_menu', 0.0):.2f} raciones"
            )
            with st.expander(titulo_ficha, expanded=False):
                ingredientes_ficha_df = pd.DataFrame(ficha.get("ingredientes", []))
                if not ingredientes_ficha_df.empty:
                    st.dataframe(
                ...
                st.markdown("**Proceso de elaboración**")
